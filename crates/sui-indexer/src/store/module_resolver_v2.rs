...
impl ModuleResolver for IndexerModuleResolverV2 {
    ...
    fn get_module(&self, id: &ModuleId) -> Result<Option<Vec<u8>>, Self::Error> {
        ...
        tracing::error!("get_module: {:?}", id);
        let package_id = ObjectID::from(*id.address()).to_vec();
        let module_name = id.name().to_string();
        tracing::error!("get_module: package_id: {:?}", package_id);
        tracing::error!("get_module: module_name: {:?}", module_name);
        ...
        .tap_err(|e| tracing::error!("get_module: error: {:?}", e))
        .context("Error reading module.")?;

        tracing::error!("get_module: module_bytes: {:?}", module_bytes);
