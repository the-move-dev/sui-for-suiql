...
async fn test_upgrade_max_packages() {
    ...
    for _ in 0..max_pub_cmd {
        packages.push((modules.clone(), dependencies.clone()));
    }
    ...
    for (package, upgrade_cap) in packages_and_upgrades {
...
async fn test_upgrade_more_than_max_packages_error() {
    ...
    let packages = vec![(modules.clone(), dependencies.clone()); 2];
